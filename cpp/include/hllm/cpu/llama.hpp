...
using LlamaConfig = model::DenseConfig;
...
class LayerKvCache final {
 public:
  ...
  [[nodiscard]] float key(std::size_t position, std::size_t head, std::size_t dimension) const;
  [[nodiscard]] float value(std::size_t position, std::size_t head, std::size_t dimension) const;
...
[[nodiscard]] Matrix rms_norm(const Matrix& input, std::span<const float> weights, float epsilon);
...
[[nodiscard]] Matrix transformer_layer(const Matrix& input, const LayerWeights& weights,
                                       const LlamaConfig& config, std::size_t first_position,
                                       LayerKvCache& cache);
[[nodiscard]] Matrix final_logits(const Matrix& hidden, std::span<const float> final_norm_weights,
